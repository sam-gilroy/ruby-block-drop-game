...
class TetrisGame
  ...
  def render_block x,y,i,a=255
    ...
    @args.outputs.labels << [grid_x + (x*boxsize), (720-grid_y) - (y*boxsize),i]
  end
  ...
  def iterate
    ...
    if k.key_down.left || k2.key_down.left
      puts "grid_space: "
      puts @curr_piece_x
      puts @curr_piece[0][@curr_piece[0].length-1]
      puts @grid[@curr_piece_x][@curr_piece[0].length]
      puts @curr_piece[0]
      @curr_piece_x-=1 if (@curr_piece_x > 0) && (@grid[@curr_piece_x-1][@curr_piece_y]==0) && (@grid[@curr_piece_x][@curr_piece[0].length-1]==0)
    end
    if k.key_down.right || k2.key_down.right
      @curr_piece_x+=1 if @curr_piece_x < @grid_w-@curr_piece.length && @grid[@curr_piece_x+@curr_piece.length][@curr_piece_y]==0
    end
