...
class TetrisGame
  ...
  def rotate_piece_right
    puts "correct change?: #{@curr_piece.reverse.transpose.map(&:reverse)}"
    puts "0 rotate: #{@curr_piece}"
    puts "0 rotate (transposed): #{@curr_piece.transpose}"
    @curr_piece = @curr_piece.transpose.map(&:reverse)
    puts "1 rotate: #{@curr_piece}"
    puts "1 rotate (transposed): #{@curr_piece.transpose}"
    @curr_piece = @curr_piece.transpose.map(&:reverse)
    puts "2 rotate: #{@curr_piece}"
    @curr_piece = @curr_piece.transpose.map(&:reverse)
    puts "3 rotate: #{@curr_piece}"
