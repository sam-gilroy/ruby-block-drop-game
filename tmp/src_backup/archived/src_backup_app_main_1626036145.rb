...
class TetrisGame
  ...
  def render_score
    @args.outputs.labels << [905,350,"Score: #{@score}",10,255,255,255,255]
  end
    ...
    for y in 0..@grid_h-1
      ...
      if full
        ...
        for i in y.downto(1) do
          for j in 0..@grid_w-1 do
            @grid[j][i] = @grid[j][i-1]
          end
        end
        for i in 0..@grid_w-1 do
          @grid[i][0] = 0
        end
      end
    end
  ...
  def rotate_piece_left
    if !curr_piece_colliding
      puts "current piece x: #{@curr_piece_x}"
      puts "current piece length: #{@curr_piece.length}"
  ...
  def is_piece_to_left
  end

  def is_piece_to_right
  end
...
def tick args
  ...
  args.state.game ||= TetrisGame.new args,k,c
  final_score = args.state.game.tick
  if args.state.game.isGameover
    args.outputs.solids << [0,0,1280,720,0,0,0]
    args.outputs.labels << [200,460,"GAME OVER",100,255,255,255,255]
    args.outputs.labels << [500,250,"Final Score: #{final_score}",10,255,255,255,255]
    args.outputs.labels << [350,200,"Press Spacebar or Start Button to Replay",5,255,255,255,255]
